...
class RPN3D(object):
    ...
    def __init__(self,
                 cls='Car',
                 single_batch_size=1,  # batch_size_per_gpu 2 to 1
                 learning_rate=0.001,
    ...
    def predict_step(self, session, data, summary=False, vis=False):
        ...
        ret_box3d = []
        ret_score = []
        for batch_idx in range(len(self.avail_gpus) * self.single_batch_size):
            # remove box with low score
            ind = np.where(batch_probs[batch_idx, :] >= cfg.RPN_SCORE_THRESH)[0]
            tmp_boxes3d = batch_boxes3d[batch_idx, ind, ...]
            tmp_boxes2d = batch_boxes2d[batch_idx, ind, ...]
            tmp_scores = batch_probs[batch_idx, ind]
    ...
    def predict_step_ros(self, session, data):
        # input:
        #     vox_feature
        #     vox_number
        #     vox_coordinate
        ...
        # print("[+] vox_feature shape : {} ".format(vox_feature.shape))  #  [1, 5836, 35, 7]
        # print("[+] vox_feature[0] shape : {} ".format(vox_feature[0].shape))  #  [5836, 35, 7]

        input_feed[self.vox_feature[0]] = vox_feature[0]
        input_feed[self.vox_number[0]] = vox_number[0]
        input_feed[self.vox_coordinate[0]] = vox_coordinate[0]

        output_feed = [self.prob_output, self.delta_output]
        probs, deltas = session.run(output_feed, input_feed)
        # print("[+] probs: {}, deltas: {}".format(probs.shape, deltas.shape)) #  DEBUG  [2,200,176,2],  [2,200,176,14]
        ...
        batch_probs = probs.reshape(
            (len(self.avail_gpus) * self.single_batch_size, -1))
        # print("[+] batch_probs max: {}, min: {}".format(batch_probs.max().round(1), batch_probs.min().round(1)))  #  DEBUG
        # print("[+] batch_probs.shape :  {}".format(batch_probs.shape))   #  DEBUG  [1, 70400]
        # NMS
        ret_box3d = []
        ret_score = []
        ...
        # print("[+] ind: {}".format(ind.shape))	#  DEBUG
        tmp_boxes3d = batch_boxes3d[0, ind, ...]
        tmp_boxes2d = batch_boxes2d[0, ind, ...]
        tmp_scores = batch_probs[0, ind]

        # print("[+] FIRST================")	#  DEBUG
        # print("[+] tmp_boxes3d: {}, tmp_scores: {}".format(tmp_boxes3d.shape, tmp_scores.shape))  #  DEBUG
        ...
        ret_box3d_score = []
        for boxes3d, scores in zip(ret_box3d, ret_score):
            # print("boxes3d: {}".format(boxes3d))  #  [x,y,z,h,w,l,rw] 
            ret_box3d_score.append(np.concatenate([np.tile(self.cls, len(boxes3d))[:, np.newaxis],
                                                   boxes3d, scores[:, np.newaxis]], axis=-1))
